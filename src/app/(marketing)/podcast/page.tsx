...
export const metadata: Metadata = {
  title: "Podcast | Leidy Abello",
  description: "Episodios sobre bienestar, autoestima y estética consciente.",
};

export default async function PodcastPage() {
  const podcastEpisodes = await sanityFetch<PodcastEpisode[]>({
    ...
    <section className="mx-auto max-w-5xl space-y-10 px-6 py-16 md:px-10">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {podcastEpisodes?.map((episode) => (
          <article key={episode.slug.current} className="rounded-2xl border border-neutral-100 bg-white p-8 shadow-sm transition hover:shadow-md">
            <p className="text-xs uppercase tracking-[0.2em] text-brand-300">
              {episode.duration}
              {episode.publishedAt && ` · ${new Date(episode.publishedAt).toLocaleDateString("es-ES", { year: 'numeric', month: 'long' })}`}
            </p>
            <h2 className="mt-3 text-2xl font-semibold">{episode.title}</h2>
            <p className="mt-3 text-sm text-neutral-600">{episode.excerpt}</p>
